...
export async function obtenerResumenMensual(mesRef: string): Promise<{ data: ResumenMensual | null; error: string | null }> {
  try {
    ...
    if (error) {
      console.error('obtenerResumenMensual:', error)
      return { data: null, error: 'No se pudo cargar el resumen' }
    }
    ...
    console.error('obtenerResumenMensual:', e)
    return { data: null, error: 'No se pudo cargar el resumen' }
  }
}
...
export async function obtenerEstadoTarjetas(): Promise<{ data: EstadoTarjetas | null; error: string | null }> {
  try {
    const supabase = await createClient()
    const { data: { user } } = await supabase.auth.getUser()
    if (!user) return { data: null, error: MSG_NO_AUTH }

    const { data: tarjetas, error: errTar } = await supabase
      .from('tarjetas')
      .select('id, nombre')
      .eq('usuario_id', user.id)

    if (errTar) {
      console.error('obtenerEstadoTarjetas tarjetas:', errTar)
      return { data: null, error: 'No se pudo cargar el estado de tarjetas' }
    }
    ...
    if (errTx) {
      console.error('obtenerEstadoTarjetas transacciones:', errTx)
      return { data: null, error: 'No se pudo cargar el estado de tarjetas' }
    }
    ...
    console.error('obtenerEstadoTarjetas:', e)
    return { data: null, error: 'No se pudo cargar el estado de tarjetas' }
  }
}
...
export async function obtenerBalanceAnual(anio: string): Promise<{ data: MesBalance[] | null; error: string | null }> {
  try {
    const supabase = await createClient()
    const { data: { user } } = await supabase.auth.getUser()
    if (!user) return { data: null, error: MSG_NO_AUTH }
    ...
    if (error) {
      console.error('obtenerBalanceAnual:', error)
      return { data: null, error: 'No se pudo cargar el balance anual' }
    }
    ...
    console.error('obtenerBalanceAnual:', e)
    return { data: null, error: 'No se pudo cargar el balance anual' }
  }
}
